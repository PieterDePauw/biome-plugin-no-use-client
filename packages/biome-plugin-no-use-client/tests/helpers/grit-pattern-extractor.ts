...
export function extractFilenamePattern(): RegExp {
	try {
		...
		if (!filenamePatternMatch)
			throw new Error(
				`Failed to extract filename pattern from GritQL file: ${GRIT_FILE_PATH}\n` +
		...
		if (error instanceof Error)
			throw new Error(
				`Error reading GritQL pattern from ${GRIT_FILE_PATH}: ${error.message}`,
			);
		throw error;
	}
}
...
export function shouldTriggerRule(filename: string, content: string): boolean {
	...
	const hasUseClient = ['"use client"', "'use client'"].some((pattern) =>
