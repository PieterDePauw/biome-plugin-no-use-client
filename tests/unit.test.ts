...
import { readFixture, shouldTriggerRule, extractFilenamePattern } from "./helpers"
...
describe('GritQL Pattern Logic - File Name Matching', () => {
	...
	const validPageFileNames = ['page.js', 'page.jsx', 'page.ts', 'page.tsx', 'app/page.tsx', 'app/dashboard/page.ts', 'src/app/users/[id]/page.jsx', 'nested/deep/structure/page.js'];

	// > Define valid layout file names for testing
	/* biome-ignore formatting: "keep on a single line" */
	const validLayoutFileNames = ['layout.js', 'layout.jsx', 'layout.ts', 'layout.tsx', 'app/layout.tsx', 'app/dashboard/layout.ts', 'src/app/users/layout.jsx', 'nested/deep/structure/layout.js']

	// > Define invalid file names that should not match as page/layout files
	/* biome-ignore formatting: "keep on a single line" */
	const invalidFileNames = ['component.tsx', 'Button.jsx', 'utils.ts', 'config.js', 'page.test.ts', 'layout.stories.tsx', 'page-component.tsx', 'layout-wrapper.tsx', 'app/components/Header.tsx', 'src/utils/helper.js']

	// > CASES THAT SHOULD BE IDENTIFIED AS PAGE FILES
	describe('should match page files', () => it.each(validPageFileNames)('matches %s', (filename) => expect(pageLayoutRegex.test(filename)).toBe(true)));

	// > CASES THAT SHOULD BE IDENTIFIED AS LAYOUT FILES
	describe('should match layout files', () => it.each(validLayoutFileNames)('matches %s', (filename) => expect(pageLayoutRegex.test(filename)).toBe(true)));

	// > CASES THAT SHOULD NOT BE IDENTIFIED AS PAGE/LAYOUT FILES
	describe('should NOT match non-page/layout files', () => it.each(invalidFileNames)('does not match %s', (filename) => expect(pageLayoutRegex.test(filename)).toBe(false)));
});
...
describe('GritQL Pattern Logic - Use Client Detection', () => {
	...
	describe('should detect use client directives', () => {
		...
		it('detects multiple directives', async () => {
			const content = await readFixture('edge-cases', 'page-multiple-use-client.tsx');
			expect(containsUseClientDirective(content)).toBe(true);
		});

		it('detects directive with whitespace', async () => {
			const content = await readFixture('edge-cases', 'layout-with-whitespace-use-client.tsx');
			expect(containsUseClientDirective(content)).toBe(true);
		});
	});
		...
		it('does not detect in commented directives', async () => {
			...
			const hasUncommentedDirective = content.split('\n')
				.some(line => {
					...
					return (trimmed.includes('"use client"') || trimmed.includes("'use client'"))
						&& !trimmed.startsWith('//');
				});
...
describe('Combined Pattern Logic', () => {
	...
	describe('should NOT trigger rule for valid cases', async () => {
		...
		it('does not trigger for component with use client', async () => {
			const content = await readFixture('valid', 'component-with-use-client.tsx');
			expect(shouldTriggerRule('app/components/Button.tsx', content)).toBe(false);
		});
	});
});
...
describe('Performance Tests - Pattern Matching', () => {
	...
	it('should handle large content efficiently', () => {
		const largeContent = 'const data = ' + JSON.stringify({
		...
		const result = regex.test('app/page.tsx') && (largeContent.includes('"use client"') || largeContent.includes("'use client'"));
		// > Get the end time
